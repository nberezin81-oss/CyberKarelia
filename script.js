(function() {
    // ---------- СТАТИЧЕСКИЕ УЧЕБНЫЕ МАТЕРИАЛЫ (без админки) ----------
    ...
    // Функция отображения страницы модуля (учебные материалы)
    ...
    function resetQuizAll() { currentIndex = 0; userAnswers = []; if(resultDetailElem) resultDetailElem.classList.add('hidden'); if(restartBtn) restartBtn.classList.add('hidden'); if(nextBtn) nextBtn.classList.add('hidden'); if(quizAreaDiv) quizAreaDiv.classList.add('hidden'); if(categorySelectionDiv) categorySelectionDiv.classList.remove('hidden'); currentCategory = null; currentQuestionsArr = []; }
    function startQuiz(cat) { currentCategory = cat; currentQuestionsArr = questionSets[cat]; currentIndex = 0; userAnswers = new Array(currentQuestionsArr.length).fill(null); categorySelectionDiv.classList.add('hidden'); quizAreaDiv.classList.remove('hidden'); resultDetailElem.classList.add('hidden'); restartBtn.classList.add('hidden'); renderQuizQuestion(); }
    function renderQuizQuestion() { const q = currentQuestionsArr[currentIndex]; progressFill.style.width = (currentIndex / currentQuestionsArr.length * 100) + '%'; let html = `<div class="question-text">${currentIndex+1}. ${escapeHtml(q.question)}</div><div class="options">`; q.options.forEach((opt,i)=>{ html += `<button class="option-btn ${userAnswers[currentIndex] === i ? 'selected' : ''}" data-idx="${i}">${escapeHtml(opt)}</button>`; }); html += '</div>'; quizContentDiv.innerHTML = html; document.querySelectorAll('.option-btn').forEach(btn=>btn.addEventListener('click',function(){ const idx = parseInt(this.dataset.idx); document.querySelectorAll('.option-btn').forEach(b=>b.classList.remove('selected')); this.classList.add('selected'); userAnswers[currentIndex] = idx; if(nextBtn) nextBtn.classList.remove('hidden'); })); nextBtn.classList.toggle('hidden', userAnswers[currentIndex] === null); }
    function showQuizResult() { let correctCount=0; const details=currentQuestionsArr.map((q,idx)=>{ const isCorrect = userAnswers[idx] === q.correct; if(isCorrect) correctCount++; return { question: q.question, userAnswer: q.options[userAnswers[idx]] || '—', correctAnswer: q.options[q.correct], isCorrect }; }); quizContentDiv.innerHTML=''; progressFill.style.width='100%'; const percent = Math.round((correctCount/currentQuestionsArr.length)*100); let detailHtml = `<h3>Результат: ${correctCount}/${currentQuestionsArr.length} (${percent}%) – ${percent>=80?'Отлично!':percent>=50?'Хорошо':'Стоит подтянуть знания'}</h3><div class="result-detail">`; details.forEach((d,i)=>{ detailHtml += `<p><strong>${i+1}. ${escapeHtml(d.question)}</strong><br>Ваш ответ: <span style="color:${d.isCorrect?'green':'red'};">${escapeHtml(d.userAnswer)}</span><br>Правильный: <span style="color:green;">${escapeHtml(d.correctAnswer)}</span></p>`; }); detailHtml += '</div>'; resultDetailElem.innerHTML = detailHtml; resultDetailElem.classList.remove('hidden'); nextBtn.classList.add('hidden'); restartBtn.classList.remove('hidden'); }
    nextBtn.addEventListener('click',()=>{ if(userAnswers[currentIndex] === null) alert('Выберите вариант'); else if(currentIndex < currentQuestionsArr.length-1){ currentIndex++; renderQuizQuestion(); } else showQuizResult(); });
    ...
    function showThreatModal(index) {
        ...
        threatModalText.innerHTML = threat.fullText;
    ...
    if(threatsGridMain) {
        ...
        threatsData.forEach((threat, idx) => {
            ...
            tile.innerHTML = `<i class="fas ${threat.icon}"></i><h3>${escapeHtml(threat.title)}</h3><p>${escapeHtml(threat.fullText.substring(0, 0))}</p>`;
    ...
    document.querySelectorAll('.close-modal, .closeAboutModal, .closeAboutModalBtn, .closeMissionModal, .closeMissionModalBtn, .closeRelevanceModal, .closeRelevanceModalBtn').forEach(btn=>btn.addEventListener('click',()=>{ document.getElementById('aboutServiceModal').style.display='none'; document.getElementById('missionModal').style.display='none'; document.getElementById('relevanceModal').style.display='none'; }));
    window.addEventListener('click',(e)=>{ if(e.target===document.getElementById('aboutServiceModal')) document.getElementById('aboutServiceModal').style.display='none'; if(e.target===document.getElementById('missionModal')) document.getElementById('missionModal').style.display='none'; if(e.target===document.getElementById('relevanceModal')) document.getElementById('relevanceModal').style.display='none'; });
})();